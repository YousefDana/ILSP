...
for pageNum in range(1, mxd.dataDrivenPages.pageCount + 1):
	...
	#Assigns different numbers of classes based on the maximum crash value
            ...
            #Exports a PDF in the style of (ILSP_Filename_Figure'PAGENUM'-1) for each page
            arcpy.AddMessage("Exporting page {0} of {1}".format(str(mxd.dataDrivenPages.currentPageID), str(mxd.dataDrivenPages.pageCount)))
            arcpy.mapping.ExportToPDF(mxd, SavingDirectory + "\CrashRateHeatmap2012to2016_EA" + EAnum + "_" + str(pageName) + "County_" + todayDate)
